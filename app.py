...
st.subheader(
    """
A simple app to analyze the sentiment of tweets.
"""
)
...
with open(
    join(dirname(realpath(__file__)), "model/sentiment-model.pkl"), "rb"
) as f:
    model = joblib.load(f)
...
if submit:
    ...
    if detect(tweet) == "sw":
        ...
        st.write(" ⚠️ The tweet is not in english language.Please make sure the input is in english language")
